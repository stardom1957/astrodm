...
class DoubleSessions:
    ...
    def __init__(self, typeSession='complete', chProg=''):
        ...
        # nombre de sessions d'observation (S)
        self.nbrS = 0
        ...
        if typeSession == 'complete':
            ...
            lstCol = ['Groupe', 'Dates_UTC', 'delai_entre_S (j)', 'Liste sessions (S)']
            ...
            'con', 'N', 'groupe', 'Dates_UTC', 'delai_entre_N', 'Liste sessions (S)', 'epoque',\
    ...
    def cree_liste_objets_Session(self):
        ...
        if self.typeSession == 'ech':
            ...
            # parcourir les dossiers de session .cheminS
            for ch_rep_session_S in lstDesRepS:
                ...
                # obs_S numéro effectif de l'obsersation
                obs_S = int(tempo.strip('S'))
                ...
                #
                lstSessions.append(Session(obs_S, 'complete'))
    
                # chemin du répertoire Sx
                lstSessions[nombreDeS].cheminS = ch_rep_session_S
                ...
                #debug lstSessions[nombreDeS].prog = path.basename(self.cheminProgramme)
    
                for repBlocCourant in listeRepBlocs:
                    # récupérer le no du bloc
                    noBlocCourant = int(path.basename(path.dirname(repBlocCourant)).split('b')[1])
                    ...
                    if ncflr_sep is None:
                        # erreur fatale
                        tampon = "doublesoutils.cree_liste_objets_Session :: Erreur fichier log REDUC manquant."
                        inscrire_dans_log(tampon)
                    ...
                    lstSessions[nombreDeS].ajouteBloc(noBlocCourant, 'complete') # nbrBlocs = noBloc = 1
     
                    # sep
                    lstSessions[nombreDeS].lstBlocs[noBlocCourant-1].sep.ncfla =  ncfla_sep
                    lstSessions[nombreDeS].lstBlocs[noBlocCourant-1].sep.ncfle =  ncfle_sep
                    lstSessions[nombreDeS].lstBlocs[noBlocCourant-1].reduc.ncflr_sep =\
                        repBlocCourant + '/sep/reduc/' + ncflr_sep
    
                    # pos
                    lstSessions[nombreDeS].lstBlocs[noBlocCourant-1].pos.ncfla =  ncfla_pos
                    lstSessions[nombreDeS].lstBlocs[noBlocCourant-1].pos.ncfle =  ncfle_pos
                    lstSessions[nombreDeS].lstBlocs[noBlocCourant-1].reduc.ncflr_pos =\
                        repBlocCourant + '/pos/reduc/' + ncflr_pos
                    
                nombreDeS += 1
            #return lstSessions, nombreDeS
            self.lstObjSession = lstSessions
            self.nbrS = nombreDeS
    ...
    def calculer_pour_publication(self):
        ...
        for index_groupe in self.groupes_de_sessions_df.index:
            ...
            str_delai =  self.groupes_de_sessions_df.loc[index_groupe, 'delai_entre_S (j)']
            lst_int_sessions_du_groupe = self.groupes_de_sessions_df.loc[index_groupe, 'Liste sessions (S)']
            ...
            if len(lst_int_sessions_du_groupe) == 1:
                ...
                index_de_no_de_session = self.moyennes_par_session_df.\
                  loc[self.moyennes_par_session_df.noSession==no_de_session].index[0]
        ...
        if self.typeSession == 'complete':
            ...
            for ses in self.lstObjSession:
                ...
                print('Session no {0} de type «{1}», contenant {2} bloc{3}'.format(\
                             ses.noSession, ses.typeSession, ses.nb, 's' if ses.nb > 1 else '') )
                print('Chemin {0} : '.format(ses.cheminS))
                print('#'*64)
                if ses.typeSession == 'ech':
                    for b in range(ses.nb):
                        print('    ' + '#' * 14)
                        print('  bloc {0}'.format(ses.lstBlocs[b].indexBloc+1))
                        print('    ' + '#' * 14)
                ...
                if ses.typeSession == 'complete':
                    for b in range(ses.nb):
                        print(NL)
                        print('        ' + '#' * 21)
                        print('         Session {0}, bloc {1}'.format(ses.noSession, ses.lstBlocs[b].indexBloc))
                        print('        ' + '#' * 21)
                        print('\r')
                        for key in ses.lstBlocs[b].__dict__.keys():
                            print("        {0} = {1}".format(key, ses.lstBlocs[b].__dict__[key]))
...
def post_reduction(type_session='complete', ch_prog=''):
    ...
    if type_session == 'complete':
        ...
        sessions = DoubleSessions(typeSession=type_session, chProg=ch_prog)
            
        # nom de la paire dans sessions
        sessions.paire = path.basename(path.dirname(ch_prog))

        # créer la liste des observations de sessions selon type_session
        # le nombre de S sera placé dans sessions.nbrS
        #sessions.lstObjSession,  sessions.nbrS = cree_liste_objets_Session(type_session, ch_prog, sessions.systeme.nom)
        sessions.cree_liste_objets_Session()

        if sessions.lstObjSession is None:
            ecrire_log_sur_disque(ncflog=ncfichier_log_post_reduction)
            # retourner None en cas d'erreur fatale
            return None
        ...
        for ses in sessions.lstObjSession:
            for bloc in ses.lstBlocs:
                # lire et extraire données du log d'acquisition et
                # d'environnement sep et pos
                extraire_Log_acq_env(ses, bloc.indexBloc, 'sep')
                extraire_mesure_brutes_logReduc(ses, bloc.indexBloc, 'sep')
                extraire_Log_acq_env(ses, bloc.indexBloc, 'pos')
                extraire_mesure_brutes_logReduc(ses, bloc.indexBloc, 'pos')
                ...
                if not bloc.validations(ses.noSession):
                    ecrire_log_sur_disque(ncflog=ncfichier_log_post_reduction)
                    return None
                ...
                bloc.reduc.calcule_rho(ses.noSession, bloc.indexBloc)
    ...
    if type_session == 'ech':
        ...
        observation = sessions.lstObjSession[0]
...
def extraire_Log_acq_env(ses, no_bloc, typeMesure):
    """
    ...
    if ses.typeSession == 'ech':
        fle = ses.lstBlocs[no_bloc-1].ech.ncfle

    if ses.typeSession == 'complete':
        if typeMesure == 'pos':
            fle = ses.lstBlocs[no_bloc-1].pos.ncfle
        if typeMesure == 'sep':
            fle = ses.lstBlocs[no_bloc-1].sep.ncfle
    with open(fle) as f:
    ...
    for ligne in lignesLues:
        if ses.typeSession == 'ech':
            ...
            if 'masque=' in ligneForm:
                ses.lstBlocs[no_bloc-1].ech.strIdMasque = (ligneForm.split('=')[1])

            # note d'acquisition dans sep
            if ('note=' in ligne) or ('note =' in ligne):
                tempo = (ligne.split('=')[1]).rstrip('\n')
                ses.lstBlocs[no_bloc-1].ech.note = tempo

            # id_filtre
            ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
            if 'filtre=' in ligneForm:
                ses.lstBlocs[no_bloc-1].ech.strIdFiltre = (ligneForm.split('=')[1])

            # 'map' mise au point
            ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
            if 'map=' in ligneForm:
                try:
                    ses.lstBlocs[no_bloc-1].ech.map = int((ligneForm.split('=')[1]))
                except ValueError:
                    print('N{0}, b{1} ech : map doit être int!'.format(ses.noSession, no_bloc.indexBloc))
                    sys.exit()

            # 'temp_to' température du to
            ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
            if 'temp_to=' in ligneForm:
                ses.lstBlocs[no_bloc-1].ech.temp_to = convertToFloat((ligneForm.split('=')[1]))
        ...
        if ses.typeSession == 'complete':
            if typeMesure == 'sep':
                ...
                if 'index_cal_e=' in ligneForm:
                    ...
                    if tempo != '':
                        ses.lstBlocs[no_bloc-1].reduc.lstIndexCalE = tempo.split(',')
                    else:
                        print("N{0}, b{1}, sep : index_cal_e est absent!".format(ses.noSession, no_bloc))
                ...
                if 'map=' in ligneForm:
                    try:
                        ses.lstBlocs[no_bloc-1].sep.map = int((ligneForm.split('=')[1]))
                    except ValueError:
                        print('N{0}, b{1} sep : map doit être int!'.format(ses.noSession, no_bloc.indexBloc))
                        sys.exit()

                # 'temp_to' température du to
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'temp_to=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].sep.temp_to = convertToFloat((ligneForm.split('=')[1]))

                # 'transp' transparance
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'transp=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].sep.transp = convertToFloat((ligneForm.split('=')[1]))

                # 'humid'
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'humid=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].sep.humid = convertToFloat((ligneForm.split('=')[1]))

                # 'press_bar' pression barométrique
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'press_bar=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].sep.press_bar = convertToFloat((ligneForm.split('=')[1]))

                # seeing
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'seeing=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].sep.seeing = (ligneForm.split('=')[1])

                # id_filtre
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'filtre=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].sep.strIdFiltre = (ligneForm.split('=')[1])

            if typeMesure == 'pos':
                ligneForm = ligne.replace(' ', '').rstrip('\n')
                # le log d'environnement de pos contient l'évaluation du quadrant de pos
                if 'eval_quadrant=' in ligneForm.lower():
                    ses.lstBlocs[no_bloc-1].pos.eval_Q = (ligneForm.split('=')[1]).upper()

                # 'map' mise au point
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'map=' in ligneForm:
                    try:
                        ses.lstBlocs[no_bloc-1].pos.map = int((ligneForm.split('=')[1]))
                    except ValueError:
                        print('N{0}, b{1} pos : map doit être int!'.format(ses.noSession, no_bloc.indexBloc))
                        sys.exit()

                # 'temp_to' température du to
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'temp_to=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].pos.temp_to = convertToFloat((ligneForm.split('=')[1]))

                # 'transp' transparance
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'transp=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].pos.transp = convertToFloat((ligneForm.split('=')[1]))

                # 'humid'
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'humid=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].pos.humid = convertToFloat((ligneForm.split('=')[1]))

                # 'press_bar' pression barométrique
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'press_bar=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].pos.press_bar = convertToFloat((ligneForm.split('=')[1]))

                # seeing
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'seeing=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].pos.seeing = (ligneForm.split('=')[1])

                # id_filtre
                ligneForm = ligne.replace(' ', '').rstrip('\n').lower()
                if 'filtre=' in ligneForm:
                    ses.lstBlocs[no_bloc-1].pos.strIdFiltre = (ligneForm.split('=')[1])
                
            # note d'environnement sep | pos
            if ('note=' in ligne) or ('note =' in ligne):
                tempo = (ligne.split('=')[1]).rstrip('\n')
                if typeMesure == 'sep':
                    ses.lstBlocs[no_bloc-1].sep.note = tempo
                if typeMesure == 'pos':
                    ses.lstBlocs[no_bloc-1].pos.note = tempo

            # remarques d'environnement sep | pos
            if ('remarques=' in ligne) or ('remarques =' in ligne):
                tempo = (ligne.split('=')[1]).rstrip('\n')
                if typeMesure == 'sep':
                    ses.lstBlocs[no_bloc-1].sep.remarques = tempo
                if typeMesure == 'pos':
                    ses.lstBlocs[no_bloc-1].pos.remarques = tempo
    ...
    if ses.typeSession == 'ech':
        ...
        for ligne in lignesLues:
            # date UTC d'acquisition (en assumant que PC d'acquisition réglé sur UTC)
            if 'Date=' in ligne:
                ses.lstBlocs[0].ech.date_utc_acq = (ligne.split('=')[1]).rstrip('\n')
        
            # heure UTC d'acquisition
            if 'Mid(UT)=' in ligne:
                ses.lstBlocs[0].ech.heure_utc_acq = (ligne.split('=')[1]).rstrip('\n')

            # nombre d'images acquises
            if 'Limit=' in ligne:
                tempo = (ligne.split('=')[1]).rstrip('\n')
                if 'Frames' in tempo:
                    ses.lstBlocs[0].ech.nbr_images_acq = tempo.split(' ')[0]
                elif 'Seconds' in tempo:
                    ses.lstBlocs[0].ech.nbr_images_acq = 'interval de ' + tempo.split(' ')[0] + ' s'


    if ses.typeSession == 'complete':
        b = no_bloc - 1
        if typeMesure == 'pos':
            fla = ses.lstBlocs[b].pos.ncfla
            with open(fla) as f:
                lignesLues = f.readlines()
            f.close()
            ...
            for ligne in lignesLues:
                # date UTC d'acquisition (en assumant que PC d'acquisition réglé sur UTC)
                if 'Date=' in ligne:
                    ses.lstBlocs[b].pos.date_utc_acq = (ligne.split('=')[1]).rstrip('\n')
            
                # heure UTC d'acquisition
                if 'Mid(UT)=' in ligne:
                    ses.lstBlocs[b].pos.heure_utc_acq = (ligne.split('=')[1]).rstrip('\n')
        
                # nombre d'images acquises
                if 'Limit=' in ligne:
                    tempo = (ligne.split('=')[1]).rstrip('\n')
                    if 'Frames' in tempo:
                        ses.lstBlocs[b].pos.nbr_images_acq = tempo.split(' ')[0]
                    elif 'Seconds' in tempo:
                        ses.lstBlocs[b].pos.nbr_images_acq = 'interval de ' + tempo.split(' ')[0] + ' s'

        if typeMesure == 'sep':
            fla = ses.lstBlocs[b].sep.ncfla
            with open(fla) as f:
                lignesLues = f.readlines()
            f.close()
            ...
            for ligne in lignesLues:
                # date UTC d'acquisition (en assumant que PC d'acquisition réglé sur UTC)
                if 'Date=' in ligne:
                    ses.lstBlocs[b].sep.date_utc_acq = (ligne.split('=')[1]).rstrip('\n')
            
                # heure UTC d'acquisition
                if 'Mid(UT)=' in ligne:
                    ses.lstBlocs[b].sep.heure_utc_acq = (ligne.split('=')[1]).rstrip('\n')
        
                # nombre d'images acquises
                if 'Limit=' in ligne:
                    tempo = (ligne.split('=')[1]).rstrip('\n')
                    if 'Frames' in tempo:
                        ses.lstBlocs[b].sep.nbr_images_acq = tempo.split(' ')[0]
                    elif 'Seconds' in tempo:
                        ses.lstBlocs[b].sep.nbr_images_acq = 'interval de ' + tempo.split(' ')[0] + ' s'
    ...
    if ses.typeSession == 'ech':
        ...
        info_masque_df = master_masques_cal_df.loc[master_masques_cal_df.id_masque==\
                                                   ses.lstBlocs[0].ech.strIdMasque]
        assert info_masque_df.index.size == 1, "Erreur, zéro ou plus d'une occurence de " + \
            ses.lstBlocs[0].ech.strIdMasque + ' dans master_masques_cal.csv'
        ...
        info_filtre_df = master_filtres_cal_df.loc[master_filtres_cal_df.id_filtre==\
                                                   ses.lstBlocs[0].ech.strIdFiltre]
        
        # placer LfplusDf, LfplusDf_sigma, Lo et Lo_sigma dans reduc
        ses.lstBlocs[0].reduc.LfplusDf = info_masque_df.loc[0, 'LfplusDf']
        ses.lstBlocs[0].reduc.LfplusDf_sigma = info_masque_df.loc[0, 'LfplusDf_sigma']
        ses.lstBlocs[0].reduc.Lo = info_filtre_df.loc[0,'Lo']
        ses.lstBlocs[0].reduc.Lo_sigma = info_filtre_df.loc[0,'Lo_sigma']
        
        assert info_filtre_df.index.size == 1, "ech : Erreur, zéro ou plus d'une occurence de " + \
            ses.lstBlocs[0].ech.strIdFiltre + ' dans master_filtre_cal.csv'
        assert ses.lstBlocs[0].ech.temp_to != None, 'environnement ech temp_to requis'
        assert ses.lstBlocs[0].ech.map != 0, 'environnement ech map requise!'

        # validation date et heure d'acquisition UTC
        d = ses.lstBlocs[0].ech.date_utc_acq
        h = ses.lstBlocs[0].ech.heure_utc_acq
        ...
        ses.lstBlocs[0].ech.dtime_utc_acq = str_dt_isot_a_TimeIsot(date=d, heure=h)
        assert ses.lstBlocs[0].ech.nbr_images_acq != 0,\
            'acq ech nombre images ou secondes acquises requis'

    if ses.typeSession == 'complete':
        b = no_bloc - 1
        if typeMesure == 'sep':
            assert ses.lstBlocs[b].sep.temp_to != None, 'environnement sep temp_to requis'
            assert ses.lstBlocs[b].sep.map != 0, 'environnement sep map requise!'

            # validation date et heure d'acquisition UTC
            d = ses.lstBlocs[b].sep.date_utc_acq
            h = ses.lstBlocs[b].sep.heure_utc_acq
            assert d != '', 'acq sep date_utc_acq requise'
            assert h != '', 'acq sep heure_utc_acq requise'
            
            # inscrire dt utc iso observation dans ...ech.dtime_utc_acq
            ses.lstBlocs[b].sep.dtime_utc_acq = str_dt_isot_a_TimeIsot(date=d, heure=h)

            assert ses.lstBlocs[b].sep.nbr_images_acq != 0, 'acq sep nombre images ou secondes acquises requis'
        
            # vérifier qu'il y a au moins un index de calibration d'échelle
            assert len(ses.lstBlocs[b].reduc.lstIndexCalE) !=0,\
                "Le fichier «" + ses.lstBlocs[b].sep.ncfle +\
                    "» ne contient pas d'index de calibration d'échelle!"

        if typeMesure == 'pos':
            assert ses.lstBlocs[b].pos.temp_to != None, 'N{0}, b{1} pos : environnement pos temp_to requis'.format(ses.noSession, b)
            assert ses.lstBlocs[b].pos.map != 0, 'N{0}, b{1} pos : environnement pos map requise!'.format(ses.noSession, b)

            # validation date et heure d'acquisition UTC
            d = ses.lstBlocs[b].pos.date_utc_acq
            h = ses.lstBlocs[b].pos.heure_utc_acq
            assert  d != '', 'N{0}, b{1} pos : acq pos date_utc_acq requise'.format(ses.noSession, b)
            assert h != '', 'N{0}, b{1} pos : acq pos heure_utc_acq requise'.format(ses.noSession, b)
            # inscrire dt utc iso seservation dans ...ech.dtime_utc_acq
            ses.lstBlocs[b].pos.dtime_utc_acq = str_dt_isot_a_TimeIsot(date=d, heure=h)

            assert ses.lstBlocs[b].pos.nbr_images_acq != 0,\
                'N{0}, b{1} pos : acq pos nombre images ou secondes acquises requis'.format(ses.noSession, b)
            assert ses.lstBlocs[b].pos.eval_Q != '',\
                "N{0}, b{1} pos : environnement pos valeur de eval_Q requise!".format(ses.noSession, b)
    

def extraire_mesure_brutes_logReduc(ses, noBloc, typeMesure):
    """
    ...
    if (ses.typeSession == 'ech') or (ses.typeSession == 'complete' and typeMesure == 'sep'):
        ncflr = ses.lstBlocs[noBloc-1].reduc.ncflr_sep
    else:
        ncflr = ses.lstBlocs[noBloc-1].reduc.ncflr_pos
    ...
    for ligne in lignesLues:
        if ses.typeSession == 'complete' and typeMesure == 'sep':
            ...
            if 'theta =' in ligne:
                ...
                ses.lstBlocs[noBloc-1].reduc.theta = convertToFloat(strTousLesNombres[0])
                ses.lstBlocs[noBloc-1].reduc.theta_sigma = convertToFloat(strTousLesNombres[1])

            # récupérer également Delta Matrix pour comparer éventuellement avec celui de pos correspondant
            if 'Delta Matrix :' in ligne:
                ses.lstBlocs[noBloc-1].reduc.delta_matrix_sep =\
        ...
        if ses.typeSession == 'complete' and typeMesure == 'pos':
            ...
            if 'Delta Matrix :' in ligne:
                ses.lstBlocs[noBloc-1].reduc.delta_matrix_pos =\
                    convertToFloat((ligne.split(':')[1].strip(' ')).rstrip('\n'))
