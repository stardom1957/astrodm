...
class Bloc:
    ...
    def validations(self, nsession):
        ...
        if self.reduc.delta_matrix_sep == self.reduc.delta_matrix_pos:
            ...
            inscrire_dans_log("   celle de sep, ce qui rend la valeur de theta incorrecte!")
            inscrire_dans_log("   Ce bloc ne sera pas inclus dans la réduction.")
...
class DoubleSessionsComplete:
    ...
    def liste_etat(self):
        ...
        print("           Système : {0}".format(self.systeme.nom))
                ...
                if self.moyennes_par_session_df.loc[idx_session, 'delaiDepuisAcqEchValide'] and \
                   self.moyennes_par_session_df.loc[idx_session, 'deltaMatrixReducValide']:
                       ...
                       for index_s in self.groupes_de_sessions_df.index:
                           if no_ses in self.groupes_de_sessions_df.loc[index_s, 'Liste sessions (S)']:
                       ...
                       if session_isolee:
                           ...
                           self.moyennes_par_session_df.loc[idx_session, 'theta_moy'],\
                           self.moyennes_par_session_df.loc[idx_session, 'theta_sigma_moy'],\
                           self.moyennes_par_session_df.loc[idx_session, 'rho_moy'],\
                           self.moyennes_par_session_df.loc[idx_session, 'rho_sigma_moy'],\
                           self.wds_Obs2,\
                           self.wds_pa2,\
                           self.wds_sep2,\
                           'TAL'
...
def extraire_Log_acq_env(ses, no_bloc, typeMesure):
            ...
            if ses.typeSession == 'ech':
                ...
                if 'map=' in ligneForm:
                    try:
                        ...
                        inscrire_dans_log('N{0}, b{1} ech : map doit être int!'.format(ses.noSession, no_bloc))
                        ses.lstBlocs[no_bloc-1].ech.valide = False
                ...
                if typeMesure == 'sep':
                    ...
                    if 'index_cal_e=' in ligneForm:
                        ...
                        if tempo != '':
                            ...
                            inscrire_dans_log("N{0}, b{1}, sep : index_cal_e est absent!".format(ses.noSession, no_bloc))
                            inscrire_dans_log("Exécutez sessions.hier() pour diagnostic.")
                            ses.lstBlocs[no_bloc-1].reduc.valide = False
                    ...
                    if 'map=' in ligneForm:
                        try:
                            ...
                            inscrire_dans_log('N{0}, b{1} sep : map doit être int!'.format(ses.noSession, no_bloc))
                            ses.lstBlocs[no_bloc-1].sep.valide = False
                ...
                if typeMesure == 'pos':
                    ...
                    if 'map=' in ligneForm:
                        try:
                            ...
                            inscrire_dans_log('N{0}, b{1} pos : map doit être int!'.format(ses.noSession, no_bloc))
                            ses.lstBlocs[no_bloc-1].pos.valide = False
    ...
    if ses.typeSession == 'ech':
        ...
        if fla is not None:
            ...
            ses.lstBlocs[0].ech.valide = False
    ...
    if ses.typeSession == 'complete':
        ...
        if typeMesure == 'pos':
            ...
            if fla is not None:
                ...
                ses.lstBlocs[b].pos.valide = False
        ...
        if typeMesure == 'sep':
            ...
            if fla is not None:
                ...
                ses.lstBlocs[b].sep.valide = False
    ...
    if ses.typeSession == 'ech' and ses.lstBlocs[0].reduc.ncflr_sep is not None:
        ...
        if info_masque_df.index.size != 1:
            ...
            inscrire_dans_log(tampon)
            ses.lstBlocs[0].reduc.valide = False
        else:
        ...
        if ses.lstBlocs[0].ech.temp_to is None:
            inscrire_dans_log('environnement ech temp_to requis')
            ses.lstBlocs[0].ech.valide = False

        if ses.lstBlocs[0].ech.map == 0:
        ...
        if ses.lstBlocs[0].ech.nbr_images_acq == 0:
    ...
    if ses.typeSession == 'complete':
        ...
        if typeMesure == 'sep':
            # environnement sep
            if ses.lstBlocs[b].sep.ncfle is not None:
                if ses.lstBlocs[b].sep.temp_to is None:
                    inscrire_dans_log('N{0}, b{1} sep : environnement sep temp_to requis'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].sep.valide = False
                if ses.lstBlocs[b].sep.map == 0:
                    inscrire_dans_log('N{0}, b{1} sep : environnement sep map requise!'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].sep.valide = False

            # validation date et heure d'acquisition UTC
            # acquisition sep
            if ses.lstBlocs[b].sep.ncfla is not None:
                d = ses.lstBlocs[b].sep.date_utc_acq
                h = ses.lstBlocs[b].sep.heure_utc_acq
                if d == '':
                    inscrire_dans_log('N{0}, b{1} acquisition sep date_utc_acq requise'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].sep.valide = False
                if h == '':
                    inscrire_dans_log('N{0}, b{1} acquisition sep heure_utc_acq requise'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].sep.valide = False
                ...
                if ses.lstBlocs[b].sep.nbr_images_acq == 0:
                    inscrire_dans_log('N{0}, b{1} acquisition sep : nombre images ou secondes acquises requis'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].sep.valide = False
        
            # vérifier qu'il y a au moins un index de calibration d'échelle
            # reduc sep
            if ses.lstBlocs[b].reduc.ncflr_sep is not None:
                if len(ses.lstBlocs[b].reduc.lstIndexCalE) == 0:
                    tampon = "N{0}, b{1} : le fichier «".format(ses.noSession, b+1) + ses.lstBlocs[b].sep.ncfle +\
                        "» ne contient pas d'index de calibration d'échelle!"
                    inscrire_dans_log(tampon)
                    ses.lstBlocs[b].reduc.valide = False

        if typeMesure == 'pos':
            # rien à faire avec ses.lstBlocs[b].reduc.ncflr_pos is not None:
            
            # environnement pos
            #
            if ses.lstBlocs[b].pos.ncfle is not None:
                if ses.lstBlocs[b].pos.temp_to is None:
                    inscrire_dans_log('N{0}, b{1} pos : environnement pos temp_to requis'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].pos.valide = False
                    
                if ses.lstBlocs[b].pos.map == 0:
                    inscrire_dans_log('N{0}, b{1} pos : environnement pos map requise!'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].pos.valide = False
                    
                if ses.lstBlocs[b].pos.eval_Q == '':
                    inscrire_dans_log("N{0}, b{1} pos : environnement pos valeur de eval_Q requise!".format(ses.noSession, b+1))
                    ses.lstBlocs[b].pos.valide = False

            # validation date et heure d'acquisition UTC
            # acquisition pos
            #
            if ses.lstBlocs[b].pos.ncfla is not None:
                d = ses.lstBlocs[b].pos.date_utc_acq
                h = ses.lstBlocs[b].pos.heure_utc_acq
                if d == '':
                    inscrire_dans_log('N{0}, b{1} pos : acq pos date_utc_acq requise'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].pos.valide = False
                if h == '':
                    inscrire_dans_log('N{0}, b{1} pos : acq pos heure_utc_acq requise'.format(ses.noSession, b+1))
                    ses.lstBlocs[b].pos.valide = False
                ...
                if ses.lstBlocs[b].pos.nbr_images_acq is None:
                    tampon = 'N{0}, b{1} pos : acq pos nombre images ou secondes acquises requis'.format(ses.noSession, b+1)
                    inscrire_dans_log(tampon)
                    ses.lstBlocs[b].pos.valide = False
...
def lire_fichiers_support(typeSession, chemin_prog):
    ...
    if typeSession == 'complete':
        ...
        ch_rep_cal = os.getcwd() + r'/med/cal_e/'
    else:
        ch_rep_cal = os.getcwd() + r'/med/cal_e/'
