...
class Reduction:
    ...
    def __init__(self):
        ...
        #
        self.intervalleTempsAcqEchValide = True

        # validité de l'intervalle de température entre toutes les mesures
        # qui servent pour la même réduction
        #
        self.intervalleTemperatureValide = True
...
class Bloc:
    ...
    def validations(self, nsession):
        ...
        Cette fonction renseigne les attributs suivants :
            ...
            self.reduc.intervalleTempsAcqEchValide :: l'intervalle de temps
             entre les acquisitions d'échelle
             et de sep <= MAX_DELAI_ENTRE_ACQUISITIONS.
             
            self.reduc.intervalleTemperatureValide :: différence de température
        ...
        if intervalle > MAX_DELAI_ENTRE_ACQUISITIONS:
            print("N{0} b{1}, cal_E et sep : intervalle de temps entre obs > {2} m!".\
                  format(nsession, self.indexBloc, MAX_DELAI_ENTRE_ACQUISITIONS))
            self.reduc.intervalleTempsAcqEchValide = False
        ...
        if self.reduc.delta_matrix_sep == self.reduc.delta_matrix_pos:
            ...
            print("   Ceci peut vouloir dire que la valeur de delta matrix n'a")
            print("   pas été transférée de la pré-réduction (REDUC) de pos à")
            print("   celle de sep. La réduction va se poursuivre, mais il est")
            print("   fort probable que la valeur de theta sera incorrecte!")
...
class DoubleSessions:
    ...
    def __init__(self, typeObs='complete', chProg=''):
        ...
        if typeObs == 'complete':
            ...
            lstCol = ['obs_prog', 'id_system', 'id_WDS', 'paire',\
             ...
             'intervalleTempsAcqEchValide',\
             'intervalleTemperatureValide', 'eval_Q', 'lstIndexCalE',\
             'E_moy', 'filtre'
            ]
            ...
            # resultat_pour_publication_df contient une seule ligne avec les valeurs moyennes
            # et autres renseignements pour le rapport des sessions
            # sera renseigné dans self.calculer_pour_publication
            self.resultat_pour_publication_df = None
        ...
        for obs in self.N:
            ...
            for b in obs.lstBlocs:
                ...
                lstChamps = [\
                         ...
                         b.reduc.intervalleTempsAcqEchValide,
                         b.reduc.intervalleTemperatureValide,
                         b.pos.eval_Q,
                         b.reduc.lstIndexCalE,
                         b.reduc.E_moy,
                         'filtre'
                         ]
                self.reductions_des_observations_df.loc[self.reductions_des_observations_df.index.size] = lstChamps


    def calculer_pour_publication(self):
        """
        Calcule les moyennes des sessions d'observations (N) du programme
        d'observations et produit le pandas dataframe
        resultat_pour_publication_df. Pour un programme, les moyennes doivent
        être calculées à partir des résultats de plusieurs sessions
        d'observations valides.
        
        Les critères qui déterminent la validité du résultat des sessions
        d'observations d'un programme sont multiples. En plus des divers
        facteurs environnementaux comme la validité de la map, les écarts de
        température, le délai depuis la dernières aquisition d'échelle, etc,
        il faut aussi que le délai entre la première et la dernière
        observations du programme soit <= MAX_DELAI_ENTRE_OBSERVATIONS.
        
        Les critères de validités au niveau des blocs ont déjà été vérifiés.
        
        Il faut bien sûr N>1 pour calculer des moyennes; avec N==2 optimal.
        Cependant, si N>2, alors c'est plus compliqué. Voici donc le
        pseudo-code de l'algorythme utilisé :
            si N=1:
                les valeurs pour N=1 sont retournées (pas de moyennes à
                calculer)
            si N>1:
                chercher le «delai le plus court» dans l'ensemble (N=1, N=x)
                  pour qui «delai le plus court» <= MAX_DELAI_ENTRE_OBSERVATIONS
                  (récursivement (N=2, Nx )...(N=3, Nx))
              si «delai le plus court» existe:
                  calculer les moyennes sur N=1 à N=x
              sinon:
                  demander à l'utilisateur de choisir l'observation
                  à utiliser pour le rapport
        
        resultat_pour_publication_df sera utilisé pour le sommaire final des
        observations, aussi bien dans le Jupyter notebook des observations que
        dans le module listeReductions.
        
        Le résultat de la validation de trouve dans le champ «État» de
        resultat_pour_publication_df.
        """
        ...
        #
        # composition du champ État
        #
        
        if self.nbrN == 1:
            # une seule session d'observations, donc on retourne simplement
            # l'enregistrement
            self.rho_moy    = self.reductions_des_observations_df.loc[0,'rho']
            self.theta_moy  = self.reductions_des_observations_df.loc[0,'theta']
            self.epoque_moy = self.reductions_des_observations_df.loc[0,'obs_epoque_B']
            self.date_obs_n = self.reductions_des_observations_df.loc[0,'obs_DH_UTC'].split('T')[0]
            
            # déterminer si la date de l'observation date de plus
            # de MAX_DELAI_ENTRE_OBSERVATIONS jours
            str_dt1 = self.reductions_des_observations_df.loc[0,'obs_DH_UTC']
            str_dt2 = datetime.now().isoformat()
            self.delai_entre_N = int(diff_dt(str_dt1, str_dt2).value)
            
            if self.delai_entre_N > MAX_DELAI_ENTRE_OBSERVATIONS:
                # état 'C' en cours
                self.etat[0] = 'T'
                self.etat[1] = 'A'
            else:
            ...
            # il y a plus d'une observations
            #
            # calculer le délai entre les observations
            #
            
            # Même si improbable, les dates Nx pourraient être dans n'importe
            # quel ordre car l'utilisateur peut librement numéroter les N
            # comme il le veut. Il faut donc créer une liste de Tupple des no
            # d'observation et des dates afin de trouver la plus longue liste
            # de dates qui sont à l'intérieure de l'intervale
            # date_première_ observation + MAX_DELAI_ENTRE_OBSERVATIONS
            
            # créer la liste de tupple pour garder la trace des no
            # d'observation après le tri
            lst_tup_dates = list()
            for idx in self.reductions_des_observations_df.index:
                lst_tup_dates.append((idx, self.reductions_des_observations_df.loc[idx,\
                                                              'obs_DH_UTC']))
    
            # trier la liste sur la date
            lst_tup_dates.sort(key=lambda tup: tup[1], reverse=True)
            
            # calculer la différence en jours (int) entre première et dernière observation
            str_dt1 = self.reductions_des_observations_df.loc[self.nbrN-1,'obs_DH_UTC']
            str_dt2 = self.reductions_des_observations_df.loc[0,'obs_DH_UTC']
            self.delai_entre_N = int(diff_dt(str_dt1, str_dt2).value)
            
            self.rho_moy    = self.reductions_des_observations_df.loc[range(0,self.nbrN),'rho'].mean()
            self.theta_moy  = self.reductions_des_observations_df.loc[range(0,self.nbrN),'theta'].mean()
            self.epoque_moy = self.reductions_des_observations_df.loc[range(0,self.nbrN),'obs_epoque_B'].mean()
            self.date_obs_n = self.reductions_des_observations_df.loc[self.nbrN-1,'obs_DH_UTC'].split('T')[0]
            
            #        
            # calculer rho_moy, theta_moy et leurs incertitudes
            #
    
            # propagation des incertitudes theta_sigma
            tempo = 0
            for val in self.reductions_des_observations_df.loc[range(0,self.nbrN),'theta_sigma']:
                tempo += math.pow(val, 2)
            self.theta_sigma_moy = math.sqrt(tempo) / self.nbrN
    
            # propagation des incertitudes rho_sigma
            tempo = 0
            for val in self.reductions_des_observations_df.loc[range(0,self.nbrN),'rho_sigma']:
                tempo += math.pow(val, 2)
            self.rho_sigma_moy = math.sqrt(tempo) / self.nbrN
            
        #
        # produire resultat_pour_publication_df avec ces résultats
        #
            
        # colonnes de resultat_pour_publication_df
        lstCol = ['prog', 'id_system', 'pr',\
        'con', 'N', 'dern_obs_utc', 'delai_entre_N', 'epoque',\
        ...
        self.resultat_pour_publication_df = pd.DataFrame(list(), columns=lstCol)
        self.resultat_pour_publication_df.loc[self.resultat_pour_publication_df.index.size] = lstData
    ...
    def calculer_moyennes_par_session(self):
        ...
        Les critères qui déterminent la validité du résultat des sessions
        d'observations d'un programme sont multiples : il existe divers
        facteurs environnementaux comme la validité de la map, les écarts de
        température, le délai depuis la dernières aquisition d'échelle, etc.
        
        Les critères de validités au niveau des blocs sont vérifiés ici.
        
        moyennes_par_session_df sera utilisé pour resultat_pour_publication_df,
        ...
        # colonnes de moyennes_par_session_df
        lstCol = ['N', 'dern_obs_utc', 'epoque_moy',\
        ...
        for session in listeDesSessions:
            ...
            if len(session_df) == 1:
                ...
                # rendu ici if deltaMatrixReducValide, intervalleTempsAcqEchValide
                # et intervalleTemperatureValide
                ...
                dern_obs_utc = session_df.loc[idx,'obs_DH_UTC'].split('T')[0]
                ...
                #
                # if deltaMatrixReducValide, intervalleTempsAcqEchValide
                ...
                dern_obs_utc = session_df.loc[interval_idx,'obs_DH_UTC']\
    ...
    def imprimerRapport(self, typeRapport='m'):
        ...
        if typeRapport == 'm':
            print(NL)
            print("MOYENNE DES OBSERVATIONS (POUR PUBLICATION)")
            print('-'*pd.get_option('display.width'))
            print(self.resultat_pour_publication_df)
            print('-'*pd.get_option('display.width'))
            print(NL)
...
def creeLstObjObservations(typeObs, cheminProgramme, nomSysteme=None):
    ...
    if typeObs == 'ech':
        ...
        lstO[0].lstBlocs[0].reduc.lstIndexCalE.append(obj_match.group(0))
        ...
        lstO[0].lstBlocs[0].reduc.ncflr_sep = tempo[0]
        return lstO, 0
            ...
            for ch_rep_session_N in lstDesRepN:
                ...
                if len(listeRepBlocs) !=0:
                    ...
                    for repBlocCourant in listeRepBlocs:
                        ...
                        lstO[obs_N-1].lstBlocs[noBlocCourant-1].sep.ncfla =  t[0]
                        lstO[obs_N-1].lstBlocs[noBlocCourant-1].sep.ncfle =  t[1]
                        
                        # déterminer log de réduction
                        tempo = repBlocCourant + '/sep/reduc'
                        lstO[obs_N-1].lstBlocs[noBlocCourant-1].reduc.ncflr_sep =\
                        ...
                        t = trouverLogsAetE(tempo, nomSysteme)
                        lstO[obs_N-1].lstBlocs[noBlocCourant-1].pos.ncfla =  t[0]
                        lstO[obs_N-1].lstBlocs[noBlocCourant-1].pos.ncfle =  t[1]
                        
                        # déterminer log de réduction
                        tempo = repBlocCourant + '/pos/reduc'
                        lstO[obs_N-1].lstBlocs[noBlocCourant-1].reduc.ncflr_pos =\
                            tempo + '/' + trouverLogReduc(tempo)
...
def maj_master_calibrations(data):
    ...
    # si plus d'une occurence, alors on arrête tout et on laisse l'us corriger la situation    
        ...
        print("ERREUR, plus d'une occurence de " + data.lstBlocs[0].reduc.lstIndexCalE[0] +\
              " sortie du programme." )
        sys.exit()
...
codes_etat_obs_dict = {
    ...
    '.A.': ['ACCEPTABLE. Programme terminé avec N=1.'],
    '.X.': ['Données INUTILISABLES. Voir carnet de note Jupyter']
}
...
MAX_DELAI_ENTRE_ACQUISITIONS = 120
