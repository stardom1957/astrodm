...
class DoubleSessionsComplete:
    ...
    def calculer_pour_publication(self):
        ...
        #TODO ou ceci maintenant = datetime.now()????
        ...
        if len(self.moyennes_par_session_df) != 0:
            ...
            for index_groupe in self.groupes_de_sessions_df.index:
                ...
                if len(lst_int_sessions_du_groupe) == 1:
                    ...
                    if nbrSessions > 1:
                        ...
                        # de MAX_DELAI_ENTRE_OBSERVATIONS en jours depuis la date unique qui
                        # se trouve dans lst_str_dates
                        delai = int(diff_dt(maintenant, lst_str_dates[0]).value)
                        if delai > MAX_DELAI_ENTRE_OBSERVATIONS:
                            self.etat = 'TAL'
                        else:
                            #TODO calculer délai pr à maintenant et date d'échéance
                            pass
                ...
                if self.moyennes_par_session_df.loc[idx_session, 'delaiDepuisAcqEchValide'] and \
                   self.moyennes_par_session_df.loc[idx_session, 'deltaMatrixReducValide']:
                       ...
                       if session_isolee:
                           ...
                           #TODO calculer délai et date d'échéance
            ...
            ############################################################################
            # TRAITEMENT DES SESSIONS N'AYANT PAS FAIT L'OBJET D'UNE RÉDUCTION VALIDE
            # cas E, X et R
            # Finalement, il faut retourner à reductions_des_observations_df et
        ...
        for ligne in lignesLues:
            if ses.typeSession == 'complete' and typeMesure == 'sep':
                ...
                if 'theta =' in ligne:
                    ...
                    bloc.reduc.theta = convertToFloat(strTousLesNombres[0])
                    bloc.reduc.theta_sigma = convertToFloat(strTousLesNombres[1])
