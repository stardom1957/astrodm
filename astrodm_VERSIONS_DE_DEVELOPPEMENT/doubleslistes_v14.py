...
def produire_lpo(ch):
    ...
    for dir_systeme in dirSystemes:
        ...
        # passer par-dessus la liste d'exclusions et traiter le répertoire
        # système
                                ...
                                if est_un_programme(chemin_prog):
                                    ...
                                    if pasDeBlocs:
                                        ...
                                        if path.exists(ncfo):
                                            ...
                                            #
                                            # cas A
                                            faire_la_reduction = False
                                            
                                            if s.reductionExecutee:
                                                # il y a eu au moins une réduction exécutée, alors
                                                # il faut déterminer si obj s est à jour,
                                                # car il est possible que des données
                                                # aient été ajoutées ou enlevée depuis la dernière post-réduction
                                                if nbr_de_rep_S == s.nbrS and s.nbrS > len(s.moyennes_par_session_df):
                                                    # cas B
                                                    faire_la_reduction = True
                                                    
                                                elif nbr_de_rep_S != s.nbrS and s.nbrS >= len(s.moyennes_par_session_df):
                                                    # possiblement nlle session ou une session enlevée
                                                    faire_la_reduction = True
                                            else:
                                                # il n'y a pas eu réduction et dans ce cas,
                                                # il n'y a pas de critère «facile» pour déterminer l'état
                                                # du programme. Il faut donc exécuter une post-réduction
                                                # pour être sûr
                                                faire_la_reduction = True
                                        ...
                                        if faire_la_reduction:
                                            s = do.post_reduction(type_session='complete', ch_prog=chemin_prog)
                                                            
                                        # ici on peut « raisonnablement » assumer que s est à jour
                                        #
                                        # on récupère les données directement de chaque sessionde l'objet s et
                                        # l'on traite selon le cas où il y a eu réduction ou non
                ...
                if boolPaire:
                    ...
                    for chemin_prog in lstProgs:
                        if est_un_programme(chemin_prog):
                            ...
                            # c'est qu'il y a eu post-réduction
                            # alors lire et traiter cet objet
                            ...
                            if len(fich) == 1:
                                ...
                                try:
                                    with open(fich[0], 'rb') as f:
                                        ...
                                        # nbr_de_rep_S et len(s.moyennes_par_session_df)
                                        ...
                                        if len(listeRepBlocs) !=0:
                                            ...
                                            if ncfle !='' and ncfla != '':
                                                ...
                                                dth_echeance = do.str_dt_isot_a_TimeIsot(dt=dtUTC_acquisition_sep) +\
                                                    do.MAX_DELAI_ENTRE_OBSERVATIONS
                                                str_dth_echeance_utc = dth_echeance.value.split('T')[0]
                                                delai = 0
                                                ...
                                                # ajouter str_dth_echeance_utc au champ echeance
...
def imprime_liste_programmes(chemin='', tri=0, impr_table_etat=True, sortie='T'):
    ...
    programmes_df = produire_lpo(chemin)
    ...
    print("  {0:>3d} en cours.".format(nbr_etat_C))
    print("  {0:>3d} avec erreur.".format(nbr_etat_X))
    print("  {0:>3d} avec erreur.".format(nbr_etat_E))
