...
version 31
Cette version est incompatibles avec les observations faite avec le Microguide
...
class DoubleSessions:
    ...
    concerné.
    '''
    def __init__(self, typeObs='complete', chSysteme=''):
        # nombre de sessions d'observation (N)
        self.nbrN = 0
        # lstObservations contiendra la liste des objets Observations
        self.lstObservations = list()
        
        if typeObs == 'complete':
            self.systeme = Systeme(chSysteme)
            # fichier _observations.csv du systeme paire
    ...
    def hierarchie(self):
        ...
        '''
        print('-'*102)
        print('{0} :'.format(self.systeme.nomSysteme))
        print(self.systeme.informations_df)
        print('-'*102)
        # puisqu'ils ne change pas durant les observations, tirer typeObservation,
        # le nom de la paire et le programme d'observation de la première observation de la liste
        if self.lstObservations[0].typeObservation == 'complete':
            print(' nomPaire = {0}'.format(self.lstObservations[0].nomPaire))
            print(' programmeObs = {0}\n'.format(self.lstObservations[0].programmeObs))
            
        for obs in self.lstObservations:
            print('#'*56)
            print('Observation no {0} de type «{1}», contenant {2} bloc{3}'.format(\
                         obs.N, obs.typeObservation, obs.nbrBlocs, 's' if obs.nbrBlocs > 1 else '') )
            print('#'*56)
            if obs.typeObservation == 'ech':
                for b in range(obs.nbrBlocs):
                    print('    ' + '#' * 10)
                    print('    le bloc {0} comprend les objets suivants :'.format(obs.lstBlocs[b].indexBloc+1))
                    print('    ' + '#' * 10)
            ...
            if obs.typeObservation == 'complete':
                for b in range(obs.nbrBlocs):
                    print('    ' + '#' * 10)
                    print('    le bloc {0} comprend les objets suivants :'.format(obs.lstBlocs[b].indexBloc))
                    print('    ' + '#' * 10)
                    for key in obs.lstBlocs[b].__dict__.keys():
                        print("        {0} = {1}".format(key, obs.lstBlocs[b].__dict__[key]))
