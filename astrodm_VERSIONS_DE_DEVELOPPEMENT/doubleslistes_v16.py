                            ...
                            for chemin_prog in lstProgs:
                                ...
                                if est_un_programme(chemin_prog):
                                    ...
                                    if pasDeBlocs:
                                        ...
                                        # d'abord récupérer les données de resultats_pour_publication_df
                                            ...
                                            if not s.lstObjSession[index_session].reductionCompletee:
                                                ...
                                                for index_bloc in range(len(s.lstObjSession[index_session].lstBlocs)):
                                                    if s.lstObjSession[index_session].lstBlocs[index_bloc].sep.valide:
                                                        Dates_obs_UTC_N = s.lstObjSession[index_session].lstBlocs[index_bloc].sep.dtime_utc_acq
...
def imprime_liste_programmes(chemin='', tri=0, impr_table_etat=True, sortie='T'):
    ...
    print(' ' + '-'*35)
    print("  {0:>3d} terminés (voir description du code du centre).".format(nbr_etat_T))
    print("  {0:>3d} en planification.".format(nbr_etat_P))
    print("  {0:>3d} sans programme | paire assigné.".format(nbr_etat_NA))
    print("  {0:>3d} en cours.".format(nbr_etat_C))
    #debug print("  {0:>3d} avec erreur.".format(nbr_etat_E))
    #TODO que faire avec nbr_etat_E?
    print("  {0:>3d} avec données inutilisables (voir journal de réduction).".format(nbr_etat_X))
