...
class Observation:
    ...
    def __init__(self, noObs, typeObservation):
        """
        Paramètres positionnels :
        noObs           -- no de l'observation (S dans les rapports et structures)
...
class DoubleSessions:
    ...
    def __init__(self, typeObs='complete', chProg=''):
        ...
        # lstObjObserv contiendra la liste des objets Observations
        self.lstObjObserv = list()
    ...
    def creer_reductions_des_observations_df(self):
        ...
        for obs in self.lstObjObserv:
            #print('Observation no. {0}'.format(obs.lstObjObserv))
        ...
        for index_groupe in self.groupes_de_sessions_df.index:
            ...
            if len(lst_int_sessions_du_groupe) == 1:
                ...
                index_de_no_de_session = self.moyennes_par_session_df.\
                  loc[self.moyennes_par_session_df.noObs==no_de_session].index[0]
    ...
    def hier(self, sortie='terminal'):
        ...
        if self.lstObjObserv[0].tobs == 'complete':
            print(' Paire     : {0}'.format(self.paire))
            print(' Programme : {0}\n'.format(self.prog))
        ...
        for obs in self.lstObjObserv:
            ...
            print('Observation no {0} de type «{1}», contenant {2} bloc{3}'.format(\
                         obs.noObs, obs.tobs, obs.nb, 's' if obs.nb > 1 else '') )
            print('Chemin {0} : '.format(obs.cheminS))
            print('#'*64)
            if obs.tobs == 'ech':
            ...
            if obs.tobs == 'complete':
                for b in range(obs.nb):
                    print(NL)
                    print('        ' + '#' * 14)
                    print('         Obs {0}, bloc {1}'.format(obs.noObs, obs.lstBlocs[b].indexBloc))
                    print('        ' + '#' * 14)
                    print('\r')
                    for key in obs.lstBlocs[b].__dict__.keys():
                        print("        {0} = {1}".format(key, obs.lstBlocs[b].__dict__[key]))
...
def post_reduction(type_obs='complete', ch_prog=''):
    ...
    - pour une observation de type 'ech', retourne un objet Observations.
    ...
    if type_obs == 'complete':
        ...
        sessions.lstObjObserv,  sessions.nbrS = creeLstObjObservations(type_obs,\
                               ch_prog, sessions.systeme.nom)

        if sessions.lstObjObserv is None:
            ecrire_log_sur_disque(ncflog=ncfichier_log_post_reduction)
            # retourner None en cas d'erreur fatale
            return None
        ...
        for obs in sessions.lstObjObserv:
            for bloc in obs.lstBlocs:
                ...
                if not bloc.validations(obs.noObs):
                    ecrire_log_sur_disque(ncflog=ncfichier_log_post_reduction)
                    return None
                ...
                bloc.reduc.calcule_rho(obs.noObs, bloc.indexBloc)
...
def creeLstObjObservations(typeObs, cheminProgramme, nomSysteme=None):
    ...
    if typeObs == 'ech':
        ...
        avec noObs=1 et ensuite créer l'unique bloc de typeObservation='ech'
        '''
        ...
        lstSessions.append(Observation(1, 'ech')) # noObs=1
        ...
        # trouver tous les répertoires Sx dans cheminProgramme
        lstDesRepS = glob.glob(cheminProgramme + '/S*/')
        nombreDeS = 0
        
        if len(lstDesRepS) == 0:
            # erreur fatale, il n'y a aucune session N
            tampon = "doublesoutils.creeLstObjObservations :: pas de session (S) dans " + cheminProgramme
            inscrire_dans_log(tampon)
            return None, 0
        ...
        for ch_rep_session_S in lstDesRepS:
            ...
            #
            lstSessions.append(Observation(obs_S, 'complete'))
            ...
            #debug lstSessions[nombreDeS].prog = path.basename(cheminProgramme)
...
def extraire_Log_acq_env(obs, no_bloc, typeMesure):
    ...
    toujours 'sep', noObs==1 et no_bloc==1.
        ...
        if obs.tobs == 'ech':
            ...
            if 'map=' in ligneForm:
                try:
                    ...
                    print('N{0}, b{1} ech : map doit être int!'.format(obs.noObs, no_bloc.indexBloc))
                    sys.exit()
            ...
            if typeMesure == 'sep':
                ...
                if 'index_cal_e=' in ligneForm:
                    ...
                    if tempo != '':
                        ...
                        print("N{0}, b{1}, sep : index_cal_e est absent!".format(obs.noObs, no_bloc))
                ...
                if 'map=' in ligneForm:
                    try:
                        ...
                        print('N{0}, b{1} sep : map doit être int!'.format(obs.noObs, no_bloc.indexBloc))
                        sys.exit()
            ...
            if typeMesure == 'pos':
                ...
                if 'map=' in ligneForm:
                    try:
                        ...
                        print('N{0}, b{1} pos : map doit être int!'.format(obs.noObs, no_bloc.indexBloc))
                        sys.exit()
    ...
    if obs.tobs == 'complete':
        ...
        if typeMesure == 'pos':
            assert obs.lstBlocs[b].pos.temp_to != None, 'N{0}, b{1} pos : environnement pos temp_to requis'.format(obs.noObs, b)
            assert obs.lstBlocs[b].pos.map != 0, 'N{0}, b{1} pos : environnement pos map requise!'.format(obs.noObs, b)

            # validation date et heure d'acquisition UTC
            d = obs.lstBlocs[b].pos.date_utc_acq
            h = obs.lstBlocs[b].pos.heure_utc_acq
            assert  d != '', 'N{0}, b{1} pos : acq pos date_utc_acq requise'.format(obs.noObs, b)
            assert h != '', 'N{0}, b{1} pos : acq pos heure_utc_acq requise'.format(obs.noObs, b)
            # inscrire dt utc iso observation dans ...ech.dtime_utc_acq
            obs.lstBlocs[b].pos.dtime_utc_acq = str_dt_isot_a_TimeIsot(date=d, heure=h)

            assert obs.lstBlocs[b].pos.nbr_images_acq != 0,\
                'N{0}, b{1} pos : acq pos nombre images ou secondes acquises requis'.format(obs.noObs, b)
            assert obs.lstBlocs[b].pos.eval_Q != '',\
                "N{0}, b{1} pos : environnement pos valeur de eval_Q requise!".format(obs.noObs, b)
