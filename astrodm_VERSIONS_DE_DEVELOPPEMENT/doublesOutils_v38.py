...
class Reduction:
    ...
    def __init__(self):
        ...
        # mapEchelleReducValide indicateur de mise au point (map) valides entre sep et
        # les map correspondant à chacun des lstIndexCalE
        # mapReducValide indicateur que map sep == map lstIndexCalE pour toutes
        # les index_cal correspondants
        #
        self.mapEchelleReducValide, self.mapReducValide = False, False
...
class Bloc:
    ...
    def validations(self, nsession):
        ...
        #
        # résultat dans self.reduc.mapEchelleReducValide
        ...
        if len(tempo) == 1:
            ...
            self.reduc.mapEchelleReducValide = True
        else:
            print("N{0} b{1}, cal_E : map non égales!".format(nsession, self.indexBloc),\
                  self.reduc.lstIndexCalE)
            # la map n'est pas la même : indiquer dans le drapeau
            self.reduc.mapEchelleReducValide = False
...
class DoubleSessions:
    ...
    def __init__(self, typeObs='complete', chProg=''):
        ...
        if typeObs == 'complete':
            ...
            lstCol = ['obs_prog', 'id_system', 'id_WDS', 'paire',\
             ...
             'press_bar', 'map', 'mapReducValide', 'deltaMatrixReducValide',\
             'intervalleTempsAcqEchValide', 'mapEchelleReducValide',\
            ...
            # df du sommaire des réductions
            # sera renseigné dans self.calcule_moyennes_et_sommaire_des_sessions
            ...
            # sera renseigné dans calcule_moyennes_et_sommaire_des_sessions
            self.etat = list('  L')
    ...
    def calcule_moyennes_et_sommaire_des_sessions(self):
        """
        Calcule les moyennes des sessions d'observations (N) du programme
        d'observatins et produit les pandas dataframe self.observations_df
        et sommaire_obs_df. Les rapports sont imprimés à partir des ces
        dataframes. Pour un programme, les moyennes doivent être calculées à
        partir des résultats de plusieurs sessions d'observations valides.
        ...
        Les critères de validités au niveau des blocs ont déjà été validés.
        ...
        sommaire_obs_df sera utilisé pour le sommaire final des observations
        aussi bien dans le Jupyter notebook des observations que dans le
        module listeReductions.
        ...
        # déterminer code état et explication dans etat
        # réfère à codes_etat_obs_dict

        # si self.nbrN == 1 et delai ok alors 'C..' (en cours)
        # sinon 'TA.' terminé avec N=1 et ACCEPTABLE
        if self.nbrN == 1 and self.delai_entre_N <=\
        ...
        # si self.nbrN > 1 et delai d'au moins 2 observations ok, alors 'TO.'
        # sinon 'TA' terminé avec N=1 et ACCEPTABLE
        if self.nbrN > 1:
            # terminée
            self.etat[0] = 'T'
    ...
    def imprimeRapport(self, typeRapport='m'):
        ...
        if typeRapport == 'm':
            print(NL)
            print("MOYENNES DES OBSERVATIONS (POUR PUBLICATION)")
        ...
        if typeRapport == 'l':
            ...
            print("\nLISTE DES BLOCS D'OBSERVATIONS POUR UNE RÉDUCTION COMPLÈTE")
            # pour chacun des N (observations)
    ...
    if type_observation == 'complete':
        ...
        for obs in sessions.N:
            for bloc in obs.lstBlocs:
                ...
                #
                # vérification de map échelle == map sep
                #
                # fatal si non égal
                ...
                if bloc.reduc.delta_matrix_sep == bloc.reduc.delta_matrix_pos:
                    bloc.reduc.deltaMatrixReducValide = True
                else:
                    print('Observation no. {0}, bloc {1} :'.\
                          format(obs.N, bloc.indexBloc))
        ...
        # création de l'objet DoubleSessions.observations_df
        ...
        #
        sessions.calcule_moyennes_et_sommaire_des_sessions()

        #################################################
        #        IMPRESSION DES RAPPORTS
        #################################################
        # rapport liste complète
        sessions.imprimeRapport('c')
...
def extrait_Log_acq_env(obs, no_bloc, typeMesure):
    """
...
MAX_DELTA_TEMPERATURE = 3
# délais entre première et dernière observation (jours)
MAX_DELAI_ENTRE_OBSERVATIONS = 7

# pourcentage acceptable de variation de la mise au point (map)
MAX_VARIATION_MAP = 0.1
